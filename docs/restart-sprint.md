...
Applied update — 24 September: Paul ran the migration and verification in Night Scout Staging. All eight consolidated security checks returned true and the function catalog confirmed the expected owner, security-definer setting, fixed search path and bootstrap-only ACL. The role remains password-disabled. Hosted callback/runtime completion and staging secret provisioning remain outstanding. Replit currently blocks the available cloud browser at its Cloudflare human-verification screen, so no hosted change or consent was attempted from this session.

## Persisted Xero readiness and summary P&L foundation — 25 September

The owner completed the staged Xero consent and atomic bootstrap on Render; the retained connection and owner-confirmed five-category mapping are therefore the durable source of truth and consent must not be repeated. This package adds an authenticated, same-store readiness boundary so Settings can show that persisted connection after navigation, redirects successful retained consent back to Settings, and prevents the UI from starting a duplicate connection while readiness is unknown. The readiness response is status-only, contains no Xero identifiers, credentials, accounts or amounts, and is gated independently from the temporary bootstrap route so bootstrap access can later be removed.

The parallel summary P&L foundation defines nine owner-reviewable leaf lines (revenue, COGS, fulfilment, performance marketing, salaries, other overheads, depreciation/amortisation, interest and tax), deterministic CM1/CM2/CM3/EBITDA/PBT/PAT totals, many-Xero-accounts-to-one-line mapping, account exclusivity, explicit not-applicable treatment and review-only AI suggestions with bounded rationale/confidence. Its SQL remains a proposal and is not applied; it does not replace the deployed five-category bootstrap mapping or access credentials. Apply and verify only the smaller readiness migration before deploying this code. A separate restricted worker run is still required to create the first accounting evidence.

## First controlled Xero refresh package — 25 September

The parallel batch now prepares the first bounded accounting refresh without repeating Xero consent. A persisted-job RPC selects exactly one active connection and complete current mapping; a durable database fence spans token rotation, five report reads and the atomic terminal evidence write. Concurrent/replayed work is denied before contacting Xero, the worker cannot use the older unfenced evidence writer, and the UI distinguishes a pending first refresh from a failed one without exposing amounts. Fourteen worker/database acceptance tests, four UI tests, frontend typecheck and the hosted build pass.

Nothing in this package has been applied to Supabase or provisioned as a paid Render service. Follow [the first-refresh runbook](xero-first-refresh-render-runbook-2026-09-25.md): apply and verify the staging migration, obtain explicit approval for Render's separate cron charge, run once with an agreed fixed period, suspend it, and verify the evidence. Recurring scheduling and rolling dates remain later work.
