...
Paul clarified that overnight execution should be cloud-hosted, independent of either Mac. The earlier local-computer question is withdrawn. This document is a reviewed deployment plan, not an active scheduled service.
...
2. Add a server-only bootstrap that reads deployment secrets, materialises owner-only temporary config/CA files for the existing launcher, cleans them up and never logs credentials. The current launcher expects local files; the cloud adapter is not yet implemented. Configure only the restricted staging intake login, Shopify development credentials and trusted TLS CA. Do not copy broad Supabase administrator/service-role secrets or any .local directory wholesale.
...
6. Propose a bounded delayed-start allowance, e.g. 02:00–02:15 local, still at most one collection per local day; never retry an uncertain write. This allowance is a recommendation requiring agreement. The current exact-minute gate can skip jobs delayed by cloud startup, so do not deploy it unchanged and describe it as reliable. Missing an overnight window should surface an alert/stale status; implementation remains required.
...
Recommend an initial **US$5/month incremental staging allowance**, subject to Paul's approval and confirmation of platform spending controls. This is a proposed budget, not an enforced cap or guaranteed bill. Stop/review if measurements project exceeding it. No new subscription, paid project or worker has been created. Do not use the old launch article's pricing as current. The [current billing guide](https://docs.replit.com/billing/deployment-pricing) describes credit-based billing; its text-rendered tariff cells were blank, so the authenticated Usage rates were used. Account balance and private billing details are deliberately excluded from this public document.

## Completion status

Completed: existing deployment inspected; official scheduling capability and current account tariff checked; cloud-only direction recorded; staging isolation, configuration adapter, historical-scope and delayed-start gaps identified. Remaining: cloud bootstrap/tests, worker project setup, connectivity check, scoped migration/grants, scope and start-window decisions, final quote/approval, deployment and overnight verification. Production and the existing Replit website remain unchanged.
