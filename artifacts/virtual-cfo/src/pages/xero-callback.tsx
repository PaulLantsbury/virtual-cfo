...
export default function XeroCallback(){
 ...
 useEffect(()=>{const p=new URLSearchParams(location.search),code=p.get('code'),state=p.get('state');
  ...
  fetch('http://localhost:4002/api/xero/callback',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({code,state})}).then(async r=>{if(!r.ok){setMessage('The local Xero snapshot was unavailable. You can close this page.');return;}const result=await r.json();const a=result.accounting;if(!a?.available){setMessage('The fixed-date Xero snapshot was read locally. No token was retained.');return;}const pounds=(minor:number)=>new Intl.NumberFormat('en-GB',{style:'currency',currency:'GBP'}).format(minor/100);setMessage(`Read-only test complete: revenue ${pounds(a.revenue)}; processing fees ${pounds(a.processingFee)}; advertising ${pounds(a.advertising)}; software ${pounds(a.software)}; included cash ${pounds(a.includedCash)}. No token or raw report was retained.`);}).catch(()=>setMessage('The local Xero service is unavailable. You can close this page.'));
 },[]);
