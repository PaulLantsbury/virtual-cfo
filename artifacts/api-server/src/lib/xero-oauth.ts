...
import {describeSelectedAccountRows} from '../../../../experiments/xero/selected-account-structure.mjs';
import {writeLocalXeroAccountStructure} from '../../../../experiments/xero/account-structure-store.mjs';
import {buildXeroMappingPreview,readLocalXeroAccountDirectory,readLocalXeroEvidenceHeader} from '../../../../experiments/xero/mapping-preview.mjs';
import {assessXeroMappingReadiness} from '../../../../experiments/xero/mapping-readiness.mjs';
...
export function xeroOAuthRuntime(env:NodeJS.ProcessEnv){
 ...
 const accountStructurePath=resolve(process.cwd(),'../../.local/xero-account-structure.json');
 const readStatusPath=resolve(process.cwd(),'../../.local/xero-read-status.json');
 const mappingPreview=env.NIGHT_SCOUT_LOCAL_BIND==='127.0.0.1'?async()=>{const [directory,mapping,evidence]=await Promise.all([readLocalXeroAccountDirectory(accountDirectoryPath),readLocalXeroAccountMapping(accountMappingPath),readLocalXeroEvidenceHeader(evidencePath)]);const readiness=assessXeroMappingReadiness({accounts:directory.accounts,mapping});return Object.freeze({testRead:evidence,mappingReadiness:readiness,...(readiness.available?buildXeroMappingPreview({accounts:directory.accounts,mapping}):{source:'xero',mappingStatus:'review_required',categories:[]})});}:undefined;
 return Object.freeze({start:()=>states.issue(config),accept:(state:string)=>states.consume(state),mappingPreview,exchange:async(code:string)=>{let phase='token_exchange';try{const {accessToken}=await exchangeReadOnlyCode({code,config});phase='tenant_discovery';const tenant=await discoverConnectedTenant({accessToken});const pinnedTenant=await pinXeroTenant(tenantPinPath,tenant);const retrievedAt=new Date().toISOString();phase='account_directory';const accounts=await discoverXeroAccounts({accessToken,tenantId:pinnedTenant.tenantId,pinnedTenantId:pinnedTenant.tenantId});await writeLocalXeroAccountDirectory(accountDirectoryPath,{tenantId:pinnedTenant.tenantId,retrievedAt,accounts});phase='report_snapshot';const snapshot=await readFixedXeroSnapshot({accessToken,tenantId:pinnedTenant.tenantId,date:snapshotDate});const mapping=await readLocalXeroAccountMapping(accountMappingPath);const structure=describeSelectedAccountRows({profitAndLoss:snapshot.profitAndLoss,balanceSheet:snapshot.balanceSheet,mapping});await writeLocalXeroAccountStructure(accountStructurePath,{retrievedAt,structure});let accounting;try{const accountTotals=extractSelectedAccountTotals({profitAndLoss:snapshot.profitAndLoss,balanceSheet:snapshot.balanceSheet,mapping});accounting=xeroAccountingView({mapping,accountTotals});}catch{accounting={available:false,reason:'selected_account_values_unavailable',revenue:null,processingFee:null,advertising:null,software:null,includedCash:null,shopifyComparison:'not_requested'};}phase='evidence_summary';const summary=xeroEvidenceSummary(snapshot,{retrievedAt});await writeLocalXeroEvidenceSummary(evidencePath,summary);return {tenant:pinnedTenant,date:snapshot.date,accountDirectory:'stored_locally',accountStructure:'stored_locally',evidence:'stored_locally',accounting};}catch(error:any){const reason=phase==='account_directory'&&['invalid_request','network_failure','upstream_refused','malformed_response'].includes(error?.safeReason)?error.safeReason:undefined;await writeLocalXeroReadStatus(readStatusPath,{phase,recordedAt:new Date().toISOString(),...(reason?{reason}:{})});throw Error('Xero read unavailable');}}});
}
