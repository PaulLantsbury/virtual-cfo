...
## Xero checkpoint — test connection and report identity only

Paul created a separate Xero test app and UK trial organisation. Local, loopback-only OAuth consent, one-tenant discovery and fixed-date read-only report snapshots succeeded. Tokens and raw report responses were discarded after each run. The local evidence boundary can retain only source/tenant identity, report date and retrieval time, base currency and four report titles; it holds no raw report, transaction, financial total, balance, account mapping or classification. The manually entered October synthetic ledger is not yet analysed. Shopify sales analysis and Xero accounting analysis are separate product views: no Shopify/Xero sales matching, comparison, variance or reconciliation is in scope. See [the Xero discovery package](xero-read-only-discovery-package-2026-09-18.md) and [synthetic test ledger](xero-matched-test-ledger-2026-09-18.md). No Xero write scope, retained credential, recurring collection, database storage, dashboard exposure or accounting write-back is authorised.
