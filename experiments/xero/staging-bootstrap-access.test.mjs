...
test('bootstrap access is staging atomic and distinct from refresh worker',()=>{
 assert.match(sql,/^BEGIN;/m);assert.match(sql,/COMMIT;\s*$/);assert.match(sql,/CREATE ROLE night_scout_xero_bootstrap_login LOGIN NOINHERIT/);assert.match(sql,/PASSWORD NULL/);assert.match(sql,/CONNECTION LIMIT 1/);assert.match(sql,/session_user <> 'night_scout_xero_bootstrap_login'/);assert.match(sql,/REVOKE ALL ON ALL TABLES IN SCHEMA xero_v1 FROM night_scout_xero_bootstrap_login/);
 ...
 assert.match(sql,/bootstrap login already exists; stop and review/);
