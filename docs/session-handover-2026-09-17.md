## 18 September continuation — first scheduled run failed; correction prepared

Read [the actual overnight evidence and correction package](first-overnight-verification-2026-09-18.md#actual-first-run--failed-before-collection-correction-prepared) first. Replit triggered the 02:00 run but launched legacy website artifact services, failed after 93.428 seconds, and recorded no nightly claim/collection. Seven candidate/financial fingerprints remain unchanged. Worker-only export correction and dated Settings failure acknowledgement are locally tested; no Replit republish or collection retry occurred. Exact worker republication is the next approval step. [Historical/daily testing proposal](testing-programme-proposal-2026-09-18.md) is prepared, not activated. Browser target-store verification remains limited by the current account's Store B-only membership. Earlier pending-first-run statements below are historical.
