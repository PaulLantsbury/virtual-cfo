...
| --- | --- | --- | --- |
| Accounting reconciliation | Maintain the operational trading report and accounting report as distinct views with an explicit bridge for timing, VAT, classification, stock and period adjustments; each difference carries provenance and status | Xero mapping, accounting period/basis, source totals and matching rules; never force equality by replacing a total. Paul must approve account mappings and adjustment authority | Xero package |
