...
Repository login and signup handlers simulate a delay and redirect to the dashboard. They do not authenticate a user. The frontend Supabase client uses the configured public key; screens retain a hardcoded demo store. The captured public database schema has no membership table or policies. Of 24 public functions, 22 use SECURITY DEFINER and two use caller permissions.
...
The existing server opportunity route was unauthenticated and used a service-role credential to read a fixed demo store. Fixing the store ID does not authenticate its caller. **The local route now returns 503 until authenticated store access is implemented**, without making an upstream request. This change is on the draft branch only; it has not changed the deployed route. Static dashboard API responses and simulated login remain demo behaviour, not production authentication.

## Proposed read-only access model

Use Supabase Auth for the first implementation (proposed choice, not an existing integration). The [local SQL proposal](../db-migrations/proposed/20260908000002_store_membership_read_access.sql) adds `store_memberships(user_id, store_id)` referencing `auth.users` and `stores`, with a composite primary key. No roles, invitation flow or client write privileges are introduced. Membership grants/removal are trusted administrative operations; no automatic demo membership or backfill is included.
...
2. Implement actual sign-in/sign-out/session expiry handling and a gated active-store selection from memberships. Never fall back to the demo UUID for an authenticated merchant. Clear store data/cache on user/store changes and logout.
...
GitHub's draft is the durable record. Nothing in this package was applied to Supabase, main or Replit. The application is still a demo and cannot yet claim authenticated tenant isolation in production.
