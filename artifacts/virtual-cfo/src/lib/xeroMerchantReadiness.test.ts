...
const connection={id:'connection_1',storeId:'store_1',tenantId:'tenant_1',status:'active',scopeVersion:'read-only-v1',createdAt:'2026-09-18T12:00:00.000Z',lastSuccessAt:null,lastFailureAt:null,mappingReviewRequired:false};
const ready={storeId:'store_1',connection,evidenceState:'ready',evidenceAsOf:'2026-09-18T12:00:00.000Z'};
...
test('merchant readiness accepts only exact value-free same-store views',()=>{
 ...
 for(const unsafe of [{...ready,amount:100},{...ready,evidenceAsOf:null},{...ready,connection:{...connection,storeId:'other'}},{...ready,evidenceState:'unknown'}])assert.equal(parseXeroMerchantReadiness(unsafe),null);
});
...
test('merchant readiness fails closed until enabled, mapped and supported',()=>{
 ...
 assert.equal(xeroMerchantReadinessView(parseXeroMerchantReadiness({...ready,evidenceState:'stale',evidenceAsOf:null}),true).canUseAccountingEvidence,false);
...
test('readiness messages do not disclose financial amounts',()=>{
 for(const state of ['checking','ready','stale','review_required','unavailable','denied'] as const){
  const value=parseXeroMerchantReadiness({...ready,evidenceState:state,evidenceAsOf:state==='ready'?'2026-09-18T12:00:00.000Z':null});
