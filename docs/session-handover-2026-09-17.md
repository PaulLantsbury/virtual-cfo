## 18 September next batch — historical/daily preparation and agreed rolling scope

Paul approved preparation of **31 completed London calendar days ending yesterday**, with missed days included in the next scheduled window and older gaps/uncertain outcomes reviewed separately. This does not change tonight's deployed fixed 17 September scope. [Historical/daily preparation and limits](testing-programme-preparation-2026-09-18.md): 132 invented orders across 14 periods, refund/recovery/cost evidence, actual disposable SQL sales-reader checks, and disabled two-week daily action plans. No synthetic staging store, source transaction, new grant or generator schedule was activated. The historical ledger still needs full candidate/import/review and profit SQL/browser integration; daily source writes still need API/capacity/recovery verification. **43/43 focused tests passed**, with independent review and a reviewed-period hardening. A pure rolling-scope planner is prepared and tested; consistent runtime propagation and exact worker deployment remain outstanding. The next genuine worker run is expected 19 September 02:00 BST; publication/readiness alone is not execution evidence.

## 18 September continuation — corrected staging worker published; next run pending

Read [the overnight evidence and approved correction record](first-overnight-verification-2026-09-18.md) first. The first scheduled run failed before collection because website artifact metadata diverted startup. Paul approved the correction; staging worker deployment `6c765555` successfully published at 10:14:45 UTC on 18 September. Hosted build smoke and workspace read-only readiness passed. Schedule, credentials, fixed 17 September reporting dates and original website remain unchanged. At 10:15:11 UTC there were zero nightly claims and only the earlier manual replay. Next expected genuine run: **19 September 02:00 BST / 01:00 UTC**. Verify actual runtime, receipt, claim and journal; do not infer collection success from publication, trigger Run now, fake time or clear reservations. No further approval is needed for read-only verification. [Historical/daily testing proposal](testing-programme-proposal-2026-09-18.md) remains unactivated and rolling scope remains a product decision. Target-store browser walkthrough remains limited by Store B-only membership. Earlier pending-publication/first-run statements below are historical.

## Cross-chat handover addition — Xero and accounting

Paul explicitly reconfirmed that the roadmap must include the **Xero connection and a Xero testing account/demo organisation**. Neither is claimed set up or verified. Confirm current Xero developer/demo options before selecting the test environment. Prepare the developer app, test organisation, OAuth/access design, account mappings and deterministic accounting fixtures; complete account/consent steps with Paul where required. Implement read-only accounting ingestion into staging, explain trading-to-accounting differences, and test reconciliation and cash inputs without double-counting Shopify revenue or platform/accounting costs. Agree mappings, cutoff, tax/cost treatment and cash assumptions before changing financial behaviour. Xero test setup/design can proceed alongside Shopify reliability work; reconciled accounting/cash evidence must precede any CFO claim depending on it. No accounting write-back is authorised by this roadmap addition.
