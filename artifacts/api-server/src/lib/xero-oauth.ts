...
export function xeroOAuthRuntime(env:NodeJS.ProcessEnv){
 ...
 return Object.freeze({start:()=>createAuthorization(config,{stateKey,nonce:randomBytes(24).toString('base64url')}),accept:(state:string)=>verifyAuthorizationState(state,{stateKey})});
}
