...
for(const viewport of ['desktop','mobile']){
 test(`${viewport}: verified sales never become actual margin or sample scenario inputs`,async()=>fixture({viewport,entry:'/margin-analysis'},async page=>{
  await metrics(page);
  const status=page.getByRole('region',{name:'Margin reporting status'});
  assert.match(await status.innerText(),/Actual margin and recovery: unavailable/);
 ...
 test(`${viewport}: free plan retains shared source amounts and honest sample gates`,async()=>fixture({viewport,plan:'free',entry:'/margin-analysis'},async page=>{
  await metrics(page);assert.equal(await page.getByRole('slider').count(),0);
  assert.equal(await page.getByText(/Upgrading does not validate or connect actual margin reporting/).count(),1);
  await page.getByRole('heading',{name:'Supporting Sample Analysis',exact:true}).click();
