...
| 6 | **Average Order Value** | Net Sales divided by qualifying order count (excl. cancelled and fully refunded) | `PARTIAL` | `orders` table → `commerceMetrics.averageOrderValue` (`total_sales / count(*)` — **frontend formula, not yet canonical**). Supabase function `average_order_value()` is implemented with the correct formula but the dashboard tile has not yet been wired to it | Shopify → `orders` → `average_order_value()` Supabase function | `orders` | `gross_sales`, `discounts`, `refunds`, `tax` |
