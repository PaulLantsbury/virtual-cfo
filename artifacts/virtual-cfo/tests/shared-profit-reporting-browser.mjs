...
for(const entry of ['/dashboard','/margin-analysis']){
 ...
 test(`${entry}: weekly period makes no profit request`,async()=>{let requests=0;await fixture({entry,initialStorage:{[`night-scout:sales-reporting:${storeId}`]:JSON.stringify({mode:'last_complete_week'})},profitRespond:scope=>{requests++;return response(scope);}},async page=>{await amounts(page,{'Gross profit':'Unavailable'});await summary(page).getByText(/complete calendar month/).waitFor();assert.equal(requests,0);});});
