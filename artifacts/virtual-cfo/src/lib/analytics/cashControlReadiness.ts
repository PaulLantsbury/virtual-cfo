...
export function cashControlReadinessView(input: CashControlReadinessInput): CashControlReadinessView {
  ...
  switch (input.state) {
    case "ready":
      return { title: "Cash evidence available", message: `Supported cash evidence is available as of ${input.asOf ?? "the recorded balance time"}.${detail}${methodology}`, tone: "ready", canShowActualCash: true, isRetained: false };
    case "stale": {
      const sameStore = !!input.retainedStoreId && input.retainedStoreId === input.storeId;
      return sameStore
