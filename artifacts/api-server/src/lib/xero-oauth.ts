...
import {buildXeroMappingPreview,readLocalXeroAccountDirectory} from '../../../../experiments/xero/mapping-preview.mjs';
...
export function xeroOAuthRuntime(env:NodeJS.ProcessEnv){
 ...
 const mappingPreview=env.NIGHT_SCOUT_LOCAL_BIND==='127.0.0.1'?async()=>buildXeroMappingPreview({accounts:(await readLocalXeroAccountDirectory(accountDirectoryPath)).accounts,mapping:await readLocalXeroAccountMapping(accountMappingPath)}):undefined;
