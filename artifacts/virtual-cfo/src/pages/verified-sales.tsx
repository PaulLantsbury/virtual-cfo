import {useStoreReporting} from '@/lib/analytics/useVerifiedBriefing';
...
export default function VerifiedSalesPage(){
 ...
 const result=useQuery({queryKey:['verified-sales',storeId,config?.currency,config?.timezone,from,to],enabled:valid&&!!config,retry:false,
  queryFn:()=>fetchVerifiedSales((name,params)=>supabase.rpc(name,params),{storeId,currency:config!.currency,from,to})});
 const data=result.data;
 const money=(pence:number)=>new Intl.NumberFormat('en-GB',{style:'currency',currency:config?.currency??'GBP'}).format(pence/100);
