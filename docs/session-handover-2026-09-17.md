...
Paul explicitly reconfirmed that the roadmap must include the **Xero connection and a Xero testing account/demo organisation**. The current official test route, read-only boundary, reconciliation rules and later decision gates are recorded in [the Xero discovery package](xero-read-only-discovery-package-2026-09-18.md). Neither a Xero app nor organisation is set up or connected. Xero test setup/design can proceed alongside Shopify reliability work; reconciled accounting/cash evidence must precede any CFO claim depending on it. No accounting write-back is authorised by this roadmap addition.
