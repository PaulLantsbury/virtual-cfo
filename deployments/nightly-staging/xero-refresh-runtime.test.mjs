...
function harness({contextMapping=mapping,keyVersion=binding.keyVersion,leaseExpiresAt=null,acquire=true,latest=[]}={}){
 ...
 const query=async(sql,params)=>{calls.push({sql,params});if(sql.includes('worker_get_refresh_context'))return {rows:[{connection_id:binding.connectionId,tenant_id:binding.tenantId,mapping_version_id:env.NIGHT_SCOUT_XERO_STAGING_MAPPING_VERSION_ID,mapping:contextMapping,ciphertext:envelope.ciphertext,encrypted_dek:envelope.encryptedDek,key_version:keyVersion,algorithm:'AES-256-GCM',version:1,lease_expires_at:leaseExpiresAt}]};if(sql.includes('worker_acquire_refresh_lease'))return {rows:[{lease_expires_at:acquire?leaseToken:null}]};if(sql.includes('worker_release_refresh_lease'))return {rows:[{released:true}]};if(sql.includes('worker_store_refresh_envelope_leased'))return {rows:[{rotated:true}]};if(sql.includes('worker_get_latest_supported_evidence'))return {rows:latest};return {rows:[{}]};};
 return {calls,query};
}
...
test('requires the persisted mapping and key version and does not contact Xero on mismatch',async()=>{
 for(const options of [{contextMapping:{...mapping,revenue:['other']}},{keyVersion:'other'},{leaseExpiresAt:new Date(Date.now()+60_000)}]){const h=harness(options),job=createStagingXeroRefreshJob({env,query:h.query,fetchImpl:()=>{throw Error('must not fetch')}});await assert.rejects(job(),/invalid/);assert.equal(h.calls.length,1);}
});
...
test('audits invalid grants safely and records a value-free failed refresh',async()=>{
 ...
 const evidence=h.calls.find(call=>call.sql.includes('worker_record_accounting_evidence'));assert.equal(evidence.params[6],'failed');assert.equal(evidence.params[7],'source_refresh_failed');assert.ok(evidence.params.slice(10).every(value=>value===null));
 assert.equal(h.calls.filter(call=>call.sql.includes('worker_release_refresh_lease')).length,1);
});
test('returns stale only for exact persisted supported evidence after refresh failure',async()=>{
 ...
 assert.equal(result.state,'stale');assert.equal(result.scope.to,'2026-09-18');assert.equal(h.calls.filter(call=>call.sql.includes('worker_release_refresh_lease')).length,1);
});
test('a fenced rotation clears ownership and report failure cannot refresh the token twice',async()=>{
 ...
 assert.equal(result.state,'failed');assert.equal(tokenCalls,1);assert.equal(h.calls.filter(call=>call.sql.includes('worker_store_refresh_envelope_leased')).length,1);assert.equal(h.calls.filter(call=>call.sql.includes('worker_release_refresh_lease')).length,0);
});
