...
function fmtDelta(n: number, unit = "£", suffix = "") {
  const sign  = n >= 0 ? "+" : "−";
  const abs   = Math.abs(n);
  const value = abs >= 1000 ? `${(abs / 1000).toFixed(0)}k` : abs.toFixed(2 * (unit === "" ? 1 : 0));
  return `${sign}${unit}${value}${suffix}`;
}
...
export default function ScenarioLab() {
  ...
  const [activePlan, setActivePlan] = useState<"balanced" | "margin" | "cash">("balanced");
  ...
  function applyPlan(plan: "balanced" | "margin" | "cash") {
    setActivePlan(plan);
    setScenario(
      plan === "balanced" ? BALANCED_GROWTH
      : plan === "margin"  ? MARGIN_RECOVERY
      : CASH_PROTECTION
    );
  }
  ...
  return (
    ...
    >
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-10">
        ...
        {/* ══ 3. CFO SUGGESTED PLANS ══════════════════════════════════════════ */}
        <SectionHeading
          title="CFO Suggested Plans"
          subtitle="Choose a recommended plan, then adjust the assumptions below."
        />
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            ...
            <div className={cn(
              ...
              <ul className="space-y-1.5">
                {[
                  "Reduce average discount by 3pp",
                  "Reallocate 15% of Meta spend to Email and Organic",
                  "Reduce shipping cost per order by £1.50",
                  "Pause low-margin acquisition campaigns",
                ].map(a => (
              ...
              <button
                onClick={() => applyPlan("margin")}
                className="w-full mt-auto text-xs font-semibold px-4 py-2 rounded-xl border border-blue-300 dark:border-blue-600 text-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-950/30 hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors"
              >
                Apply this plan
              </button>
            </div>
            ...
            <div className={cn(
              ...
              <button
                onClick={() => applyPlan("cash")}
                className="w-full mt-auto text-xs font-semibold px-4 py-2 rounded-xl border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-800/30 hover:bg-slate-100 dark:hover:bg-slate-700/30 transition-colors"
              >
                Apply this plan
              </button>
            </div>

            {/* Balanced Growth Plan — recommended */}
            <div className={cn(
              ...
              <ul className="space-y-1.5">
                {[
                  "Reduce blanket discounts by 2pp",
                  "Shift 10% of Meta spend to Email",
                  "Improve full-price order mix",
                  "Keep Google Shopping spend stable",
                ].map(a => (
                  <li key={a} className="flex items-start gap-2 text-xs text-muted-foreground">
              ...
              <button
                onClick={() => applyPlan("balanced")}
                className="w-full mt-auto text-xs font-semibold px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white transition-colors shadow-sm shadow-emerald-500/20"
              >
                Apply this plan
              </button>
            </div>
        ...
        <div className="bg-card rounded-2xl shadow-sm border border-border/50 p-6 space-y-5">
          <div className="flex items-start gap-3">
            ...
            <p className="text-sm text-foreground leading-relaxed">
              This plan is recommended because Meta CAC is the largest margin drag, discounting has increased, and cash runway is
              tightening. It improves contribution without requiring additional stock investment.
            </p>
          </div>
        ...
        <PremiumBlurPreview
          ...
          ctaDescription="Model any combination of growth, margin and cash levers and see the combined financial impact in real time."
          ghostContent={
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2 mt-6 pt-5 border-t border-border/40">
            <button
              onClick={() => { setScenario(BALANCED_GROWTH); setActivePlan("balanced"); }}
              className="inline-flex items-center gap-1.5 text-xs font-semibold px-4 py-2 rounded-xl border border-border text-muted-foreground hover:text-foreground hover:border-foreground/30 transition-colors"
            >
              <RefreshCw className="w-3.5 h-3.5" /> Reset scenario
            </button>
            <button
              className="inline-flex items-center gap-1.5 text-xs font-semibold px-4 py-2 rounded-xl border border-border text-muted-foreground hover:text-foreground hover:border-foreground/30 transition-colors"
              title="Save scenario (coming soon)"
            >
              <Save className="w-3.5 h-3.5" /> Save scenario
            </button>
            <button
              className="inline-flex items-center gap-1.5 text-xs font-semibold px-4 py-2 rounded-xl border border-border text-muted-foreground hover:text-foreground hover:border-foreground/30 transition-colors"
              title="Compare scenarios (coming soon)"
            >
        ...
        {/* ── Imported Assumptions Panel ───────────────────────────────────── */}
        <div className="bg-card rounded-2xl shadow-sm border border-border/50 p-6">
          <div className="flex items-start justify-between gap-4 mb-4">
            <div>
              <h3 className="font-semibold text-base text-foreground">Imported from page sliders</h3>
              <p className="text-xs text-muted-foreground mt-0.5">Assumptions pulled from your current page-level scenarios.</p>
            </div>
            {!isPro && (
              <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2.5 py-1 rounded-full bg-primary/10 text-primary border border-primary/20 uppercase tracking-wider shrink-0">
                PRO
              </span>
            )}
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: "Revenue",      base: fmtK(BASE_REVENUE),      scen: fmtK(out.revenue),      delta: fmtDelta(out.revenueDelta),          pos: out.revenueDelta >= 0      },
            { label: "Contribution", base: fmtK(BASE_CONTRIBUTION),  scen: fmtK(out.contribution),  delta: fmtDelta(out.contributionDelta),      pos: out.contributionDelta >= 0 },
            { label: "EBITDA",       base: fmtK(BASE_EBITDA),        scen: fmtK(out.ebitda),        delta: fmtDelta(out.ebitdaDelta),            pos: out.ebitdaDelta >= 0       },
            { label: "Cash Balance", base: fmtK(BASE_CASH),          scen: fmtK(out.cash),          delta: fmtDelta(out.cashDelta),              pos: out.cashDelta >= 0         },
          ].map(c => (
            <OutputCard key={c.label} label={c.label} base={c.base} scenario={c.scen} delta={c.delta} positive={c.pos} />
          ))}
        </div>

        {/* Detailed outputs — Pro */}
        <PremiumBlurPreview
          title="Full scenario outputs"
          subtitle="Cash runway, working capital, CAC payback and contribution per order."
          isPro={isPro}
          ctaTitle="Unlock detailed scenario outputs"
          ctaDescription="See how the scenario affects cash runway, working capital drag and CAC payback."
          ghostContent={
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {["Cash Runway", "Working Capital Drag", "CAC Payback", "Contribution per Order"].map(l => (
                <div key={l} className="bg-secondary/40 rounded-xl p-4">
                  ...
                  <div className="mt-2 h-6 rounded-lg bg-border/40" />
                </div>
              ))}
            </div>
          }
        >
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <OutputCard
              ...
              delta={`${fmtDeltaRaw(out.cpoDelta, 2)} £/order`}
              positive={out.cpoDelta >= 0}
            />
          </div>
        </PremiumBlurPreview>

        {/* ══ 7. CONTRIBUTION BRIDGE ══════════════════════════════════════════ */}
        <PremiumBlurPreview
              ...
              <BarChart data={BRIDGE_DATA} barCategoryGap="30%" margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                ...
                <Bar dataKey="value" stackId="a" radius={[4, 4, 0, 0]}>
                  {BRIDGE_DATA.map((entry) => (
                    <Cell key={entry.name} fill={BRIDGE_COLORS[entry.type]} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
        ...
        >
          <div className="flex items-start gap-3 mb-5">
            ...
            <p className="text-sm text-foreground leading-relaxed">
              Profit improves by £42k, while cash improves by £64k because inventory reduction releases cash as well as improving operating performance.
            </p>
          </div>
            ...
            {[
              {
                ...
                label: "Profit uplift",
                value: "+£42,000/month",
                text:  "Contribution improves through better margin and marketing efficiency.",
              },
              {
                icon: Shield, color: "blue",
                label: "Cash uplift",
                value: "+£64,000",
                text:  "Cash improves because working capital drag reduces.",
              },
              {
                icon: Zap, color: "indigo",
                label: "Runway extension",
                value: "+0.8 months",
                text:  "Cash cover improves from 3.4 to 4.2 months.",
              },
              {
                icon: CheckCircle, color: "green",
                label: "Risk movement",
                value: "Lower",
                text:  "Scenario reduces both margin risk and cash risk.",
              },
        ...
        {/* ══ 9. SCENARIO QUALITY SCORE ════════════════════════════════════════ */}
        <PremiumBlurPreview
          title="Scenario Quality Score"
          isPro={isProBridge}
          ctaTitle="Unlock scenario quality scoring"
          ctaDescription="See a structured quality assessment of your scenario across profit, cash, risk and payback."
        >
          <div className="flex flex-col sm:flex-row sm:items-center gap-6 mb-6">
            ...
            <div className="flex-1">
              <p className="text-sm text-foreground leading-relaxed mb-3">
                This scenario improves profit, cash and implementation risk without relying heavily on new customer acquisition.
              </p>
          ...
          <div className="flex items-start gap-2 p-3 rounded-xl bg-secondary/50">
            <Info className="w-4 h-4 text-muted-foreground shrink-0 mt-0.5" />
            <p className="text-xs text-muted-foreground leading-snug">
              Scenario quality is based on profit impact, cash impact, implementation complexity and confidence level.
            </p>
          </div>
        </PremiumBlurPreview>
          ...
          ghostContent={
            <div className="space-y-3">
              <div className="grid grid-cols-5 gap-3 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground pb-2 border-b border-border/40">
                {["Action", "Impact", "Confidence", "Effort", "Timing"].map(h => <span key={h}>{h}</span>)}
              </div>
              ...
              <thead>
                <tr className="border-b border-border/40">
                  {["Action", "Impact", "Confidence", "Effort", "Timing"].map(h => (
                    <th key={h} className="text-left text-[10px] font-semibold uppercase tracking-wider text-muted-foreground pb-3 pr-4 first:pl-0">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {IMPL_ACTIONS.map(row => (
                  <tr key={row.action} className="border-b border-border/30 last:border-0">
                    <td className="py-3 pr-4 text-sm font-medium text-foreground">{row.action}</td>
                    <td className="py-3 pr-4 text-sm text-emerald-600 dark:text-emerald-400 font-semibold whitespace-nowrap">{row.impact}</td>
        ...
        {/* ══ 11. AI CFO RECOMMENDATION ════════════════════════════════════════ */}
        <PremiumBlurPreview
          title="AI CFO Recommendation"
          isPro={isProImpl}
          ctaTitle="Unlock the AI CFO recommendation"
          ctaDescription="Get a personalised recommendation and action plan from your virtual CFO."
        >
          <div className="flex items-start gap-3 mb-5">
            <div className="w-9 h-9 rounded-full bg-primary/10 flex items-center justify-center shrink-0">
              <Sparkles className="w-5 h-5 text-primary" />
            ...
            <p className="text-sm text-foreground leading-relaxed">
              Start with the Balanced Growth Plan. It delivers a meaningful contribution uplift while protecting cash runway and
              avoiding over-reliance on new paid acquisition. Prioritise discount discipline, Meta budget reallocation and inventory
              control before increasing growth spend.
            </p>
          </div>
          ...
          <ul className="space-y-3">
            {[
              ...
              <li key={bullet} className="flex items-start gap-3 p-3 rounded-xl bg-secondary/50">
