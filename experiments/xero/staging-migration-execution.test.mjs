...
test('staging Xero installer is atomic, one-shot and preserves the worker boundary',()=>{
  ...
  assert.doesNotMatch(executable,/GRANT\s+[^;]+\s+TO\s+(worker|service_role)\b/i);
...
test('installer keeps credential envelopes browser-inaccessible and mapping reads member-scoped',()=>{
  ...
  assert.match(sql,/accounting_evidence[\s\S]*?ENABLE ROW LEVEL SECURITY/);
  assert.match(sql,/xero accounting evidence is append-only/);
  assert.doesNotMatch(sql,/GRANT SELECT ON[^;]*accounting_evidence/);
