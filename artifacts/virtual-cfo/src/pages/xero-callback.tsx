...
export default function XeroCallback(){
 ...
 useEffect(()=>{const p=new URLSearchParams(location.search),code=p.get('code'),state=p.get('state');
  ...
  fetch('http://localhost:4002/api/xero/callback',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({code,state})}).then(async r=>r.ok?setMessage('The fixed-date Xero snapshot was read locally. No token was retained.'):setMessage('The local Xero snapshot was unavailable. You can close this page.')).catch(()=>setMessage('The local Xero service is unavailable. You can close this page.'));
 },[]);
